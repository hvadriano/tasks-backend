pipeline {
        ...
        stage ('Sonar Analisys') {
            ...
            steps {
                withSonarQubeEnv('SONAR_LOCAL') {
                    bat "${scannerHome}/bin/sonar-scanner -e -Dsonar.projectKey=DeployBack -Dsonar.host.url=http://localhost:9000 -Dsonar.login=c40233f40359245af452f194d01817f52ae553b6 -Dsonar.java.binaries=target -Dsonar.coverage.exclusions=**/.mvn/**,**/src/test/**,**/model/**,**Application.java"
                }
            }
        }
        stage ('Quality Gate') {
    ...
    post {
        ...
        unsuccessful {
            emailext attachLog: true, body: 'See the attach log below', subject: 'Build $BUILD_NUMBER has failed', to: 'adriano.savoir+jenkins@gmail.com'
        }
        fixed {
            emailext attachLog: true, body: 'See the attach log below', subject: 'Build is fine!!', to: 'adriano.savoir+jenkins@gmail.com'
        }
    }
}
